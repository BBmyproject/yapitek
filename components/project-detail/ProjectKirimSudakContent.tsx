...
import { KIRIM_SUDAK_MEDIA } from "@/lib/kirim-sudak-media";
import { getTranslations } from "next-intl/server";
import type { IconType } from "react-icons";
import { MdGroups, MdLocalCafe, MdMenuBook } from "react-icons/md";
...
export async function ProjectKirimSudakContent() {
  const t = await getTranslations("ProjectKirimSudak");
    ...
    >
      <RevealFadeUp>
        <section
          ...
          aria-labelledby="kirim-stats-heading"
        >
          <div className="mx-auto max-w-[1200px]">
            <h2 id="kirim-stats-heading" className="sr-only">
              {t("statsSectionLabel")}
            </h2>
              ...
              {stats.map((item, index) => {
                ...
                return (
                  <div
                    ...
                    className="flex flex-col items-center gap-4 border border-[#d1dfe0] bg-white/80 px-4 py-7 text-center shadow-sm sm:py-8"
                  >
                    <span
        ...
        <section className="border-t border-[#d1dfe0] bg-[#eef4f4]/40 px-6 py-16 lg:px-10 lg:py-24">
          <div className="mx-auto grid max-w-[1200px] gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
            <div className="relative order-2 aspect-[4/3] overflow-hidden bg-[#d1dfdf]/50 lg:order-1">
              <img
                src={KIRIM_SUDAK_MEDIA.architecture}
                alt={t("archImageAlt")}
                width={1200}
                height={900}
                className="h-full w-full object-cover"
      ...
      <RevealFadeUp>
        <section className="border-t border-[#1f3a40] bg-[#1f3a40] px-6 py-16 text-white lg:px-10 lg:py-20">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-6 text-sm leading-relaxed text-white/90 md:text-lg md:leading-relaxed">
              {t("experienceBody")}
            </p>
          </div>
        </section>
