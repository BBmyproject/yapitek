...
import { SEHITLIK_MEDIA } from "@/lib/sehitlik-media";
...
const STAT_ICONS: IconType[] = [MdPublic, MdGroups];

export async function ProjectSehitlikContent() {
  const t = await getTranslations("ProjectSehitlik");
    ...
    >
      <RevealFadeUp>
        <section
          ...
          aria-labelledby="sehitlik-stats-heading"
        >
          <div className="mx-auto max-w-3xl">
            <h2 id="sehitlik-stats-heading" className="sr-only">
              {t("statsSectionLabel")}
            </h2>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {stats.map((item, index) => {
                const Icon = STAT_ICONS[index] ?? MdPublic;
                return (
                  <div
                    key={item.label}
