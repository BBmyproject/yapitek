...
import { EVART_1_MEDIA } from "@/lib/evart-1-media";
import { getTranslations } from "next-intl/server";
import type { IconType } from "react-icons";
import { MdBusiness, MdStorefront } from "react-icons/md";
import { TbRulerMeasure } from "react-icons/tb";

type StatItem = { value: string; label: string; detail: string };

const STAT_ICONS: IconType[] = [TbRulerMeasure, MdBusiness, MdStorefront];

export async function ProjectEvart1Content() {
  const t = await getTranslations("ProjectEvart1");
    ...
    >
      <RevealFadeUp>
        <section
          ...
          aria-labelledby="evart1-stats-heading"
        >
          <div className="mx-auto max-w-5xl">
            <h2 id="evart1-stats-heading" className="sr-only">
              {t("statsSectionLabel")}
            </h2>
        ...
        <section className="border-t border-[#d1dfe0] bg-[#eef4f4]/40 px-6 py-16 lg:px-10 lg:py-24">
          <div className="mx-auto grid max-w-[1200px] gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
            <div className="relative order-2 aspect-[4/3] overflow-hidden bg-[#d1dfdf]/50 lg:order-1">
              <img
                src={EVART_1_MEDIA.architecture}
                alt={t("archImageAlt")}
                width={1200}
                height={900}
                className="h-full w-full object-cover"
