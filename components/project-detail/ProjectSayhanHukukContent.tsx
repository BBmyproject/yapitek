...
import { SAYHAN_HUKUK_MEDIA } from "@/lib/sayhan-hukuk-media";
...
const STAT_ICONS: IconType[] = [MdBusiness, MdApartment];

export async function ProjectSayhanHukukContent() {
  const t = await getTranslations("ProjectSayhanHukuk");
    ...
    >
      <RevealFadeUp>
        <section
          ...
          aria-labelledby="sayhan-stats-heading"
        >
          <div className="mx-auto max-w-3xl">
            <h2 id="sayhan-stats-heading" className="sr-only">
              {t("statsSectionLabel")}
            </h2>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {stats.map((item, index) => {
                const Icon = STAT_ICONS[index] ?? MdBusiness;
                return (
                  <div
                    key={item.label}
        ...
        <section className="px-6 py-16 lg:px-10 lg:py-24">
          <div className="mx-auto grid max-w-[1200px] gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
            ...
            <div className="relative aspect-[4/3] overflow-hidden bg-[#d1dfdf]/50">
              <img
                src={SAYHAN_HUKUK_MEDIA.concept}
