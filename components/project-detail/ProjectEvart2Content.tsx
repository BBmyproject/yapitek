...
import { EVART_2_MEDIA } from "@/lib/evart-2-media";
...
const STAT_ICONS: IconType[] = [TbRulerMeasure, MdStorefront, MdBusiness];

export async function ProjectEvart2Content() {
  const t = await getTranslations("ProjectEvart2");
    ...
    >
      <RevealFadeUp>
        <section
          ...
          aria-labelledby="evart2-stats-heading"
        >
          <div className="mx-auto max-w-5xl">
            <h2 id="evart2-stats-heading" className="sr-only">
              {t("statsSectionLabel")}
            </h2>
        ...
        <section className="border-t border-[#d1dfe0] bg-[#eef4f4]/40 px-6 py-16 lg:px-10 lg:py-24">
          <div className="mx-auto grid max-w-[1200px] gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
            <div className="relative order-2 aspect-[4/3] overflow-hidden bg-[#d1dfdf]/50 lg:order-1">
              <img
                src={EVART_2_MEDIA.architecture}
                alt={t("archImageAlt")}
                width={1200}
                height={900}
                className="h-full w-full object-cover"
