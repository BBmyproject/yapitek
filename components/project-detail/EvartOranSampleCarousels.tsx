...
function UnitCarousel({
  ...
  return (
    ...
    >
      ...
      <h2
        id={headingId}
        className="px-4 pb-6 pt-1 md:pt-6 text-center font-serif text-3xl tracking-tight text-[#1f3a40] md:text-5xl md:pb-10"
      >
        {t(titleKey)}
      </h2>
        ...
        >
          <div className="flex touch-pan-y gap-6">
            {slides.map((slide, index) => (
              <div
                key={slide.key}
                className={`relative h-[240px] shrink-0 overflow-hidden bg-[#d1dfdf]/40 md:h-[480px] ${slideWidthClass()}`}
              >
                <img
                  src={slide.src}
                  alt={
