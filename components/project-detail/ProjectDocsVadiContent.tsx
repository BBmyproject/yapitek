...
import { DOCS_VADI_MEDIA } from "@/lib/docs-vadi-media";
...
type AccessRow = { place: string; time: string };
...
export async function ProjectDocsVadiContent() {
  const t = await getTranslations("ProjectDocsVadi");
  const stats = t.raw("stats") as StatItem[];
  const archBullets = t.raw("archBullets") as string[];
  const socialList = t.raw("socialList") as string[];
  const locationAccess = t.raw("locationAccess") as AccessRow[];
    ...
    >
      <RevealFadeUp>
        <section
          ...
          aria-labelledby="docsvadi-stats-heading"
        >
          <div className="mx-auto max-w-[1200px]">
            <h2 id="docsvadi-stats-heading" className="sr-only">
              {t("statsSectionLabel")}
            </h2>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
              {stats.map((item, index) => {
                const Icon = STAT_ICONS[index] ?? MdMeetingRoom;
                return (
                  <div
                    key={item.label}
        ...
        <section className="border-t border-[#d1dfe0] bg-[#eef4f4]/40 px-6 py-16 lg:px-10 lg:py-24">
          <div className="mx-auto grid max-w-[1200px] gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
            <div className="relative order-2 aspect-[4/3] overflow-hidden bg-[#d1dfdf]/50 lg:order-1">
              <img
                src={DOCS_VADI_MEDIA.architecture}
                alt={t("archImageAlt")}
                width={1200}
                height={900}
                className="h-full w-full object-cover"
        ...
        <section className="px-6 py-16 lg:px-10 lg:py-24">
          <div className="mx-auto max-w-[1200px]">
            ...
            <div className="mx-auto mt-12 max-w-2xl text-center">
              ...
              <ul className="mx-auto mt-8 max-w-md space-y-2.5 text-left text-sm leading-relaxed text-[#1f3a40]/88 md:text-base">
                {socialList.map((line) => (
              ...
              <p className="mt-10 font-serif text-lg leading-relaxed text-[#1f3a40] md:text-xl">
                {t("socialClosing")}
              </p>
            </div>
          </div>
          ...
          <div className="mx-auto grid max-w-[1200px] gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
            <div>
              ...
              <dl className="mt-8 space-y-4 border border-[#d1dfe0] bg-white/80 p-6">
                {locationAccess.map((row) => (
                  <div
                    key={row.place}
                    className="flex items-baseline justify-between gap-4 border-b border-[#d1dfe0]/80 pb-4 last:border-0 last:pb-0"
                  >
                    <dt className="text-sm font-medium text-[#1f3a40]/85">
                      {row.place}
                    </dt>
                    <dd className="shrink-0 font-serif text-lg text-[#0f677d] md:text-xl">
                      {row.time}
                    </dd>
                  </div>
                ))}
      ...
      <RevealFadeUp>
        <section className="border-t border-[#d1dfe0] bg-[#eef4f4]/40 px-6 py-16 lg:px-10 lg:py-24">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <ul className="mx-auto mt-8 max-w-lg space-y-3 text-left text-sm text-[#1f3a40]/90 md:text-base">
